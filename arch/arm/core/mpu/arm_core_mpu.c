...
#if defined(CONFIG_ARM_NSC_REGION_IN_RAM)
		{
		/* Special RAM area for Non-Secure Callable entry text */
		.start = (uint32_t)&__sg_start,
		.size = (uint32_t)&__nsc_size,
		.attr = K_MEM_PARTITION_P_RX_U_RX,
		},
#endif /* CONFIG_ARM_NSC_REGION_IN_RAM */
#if defined(CONFIG_CODE_DATA_RELOCATION_SRAM)
	{
