...
#ifdef CONFIG_SOC_ATM_SYSTICK_COUNTFLAG_WORKAROUND
/*
 * Workaround for the SysTick VAL=0 race condition.
 *
 * Hardware sequence when the counter is at 1 as VAL=0 is written:
 *   1. Hardware detects VAL write, clears COUNTFLAG.
 *   2. VAL=0 write commits: reloads counter from LOAD.
 *   3. Hardware detects natural 1→0 countdown and sets COUNTFLAG again.
 *
 * The result is COUNTFLAG=1 even though we just reset the counter.
 * The next elapsed() call inside the ISR would then incorrectly add
 * last_load to overflow_cyc, causing a spurious ~1 s time jump.
 *
 * Fix: Check the val2 is less than threshold to execute the workaround.
 * According simulation, the exection time form Val2 to Systick->VAL=0 is about
 * 4 systick cycle, set the threshold to 8.
 * Wait until VAL has counted down past the race window,
 * confirming the counter has reloaded cleanly from the new LOAD.
 * Then read CTRL: if COUNTFLAG is still set the race occurred and
 * the remaining val2 cycles (from the natural wrap that happened
 * between the val2 read and VAL=0) must be credited to cycle_count.
 */
#define WAIT_RELOAD_TIMES 10
#define WAIT_RELOAD_VAL   8
	if (val2 <= WAIT_RELOAD_VAL) {
		for (int _i = 0; SysTick->VAL < WAIT_RELOAD_VAL && _i < WAIT_RELOAD_TIMES; _i++) {
			/* spin until counter is safely past the reload edge */
		}
		uint32_t ctrl_after = SysTick->CTRL; /* clears COUNTFLAG as a side-effect */
		if (ctrl_after & SysTick_CTRL_COUNTFLAG_Msk) {
			/* A natural wrap occurred after val2 was read.  Add the val2
			 * cycles that were not captured by the val1-val2 subtraction. */
			cycle_count += val2;
		}
	}
#endif
