...
class ImgtoolSigner(Signer):
    ...
    def sign(self, command, build_dir, build_conf, formats):
        ...
        swap_using_offset = build_conf.getboolean(
            'CONFIG_MCUBOOT_BOOTLOADER_MODE_SWAP_USING_OFFSET', False)
        align, addr, size = self.edt_flash_params(command, flash,
                                                   swap_using_offset=swap_using_offset)
    ...
    @staticmethod
    def edt_flash_params(cmd, flash, swap_using_offset=False):
        """Return (align, addr, size) imgtool parameters from the devicetree.

        When swap_using_offset is True (Swap-with-Offset MCUboot mode), slot0
        size is used instead of slot1 size.  In that mode slot1 is one
        erase-block larger than slot0 and the OTA layer writes the upgrade
        image into slot1 starting at offset erase-block-size, so the effective
        usable image area equals slot0_size.
        """
        ...
        #
        # Exception: in swap-using-offset mode slot1 is one erase-block LARGER
        # than slot0 (the extra sector is working space for the swap algorithm).
        # The OTA layer writes the upgrade image into slot1 starting at offset
        # erase-block-size, so the effective usable image area equals slot0_size.
        ...
        # In swap-using-offset mode always use slot0_partition size (see note above).
        if swap_using_offset:
